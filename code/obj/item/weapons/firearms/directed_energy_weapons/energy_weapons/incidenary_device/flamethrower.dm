...
/obj/item/firearm/incidenary/flamethrower
	name = "flamethrower"
...
/obj/item/firearm/incidenary/flamethrower/return_air(direct = FALSE)
	return src.gastank?.return_air()

/obj/item/firearm/incidenary/flamethrower/assembled
	name = "flamethrower"
	icon = 'icons/obj/items/weapons.dmi'
	inhand_image_icon = 'icons/mob/inhand/hand_guns.dmi'
	desc = "You are a firestarter!"
...
/obj/item/tank/jetpack/backtank
	...
	c_flags = ONBACK
	var/obj/item/firearm/incidenary/flamethrower/backtank/linkedflamer
	inventory_counter_enabled = 1
	move_triggered = 1
	...
	attackby(obj/item/W, mob/user)
		if (src.loc == user && W != linkedflamer && istype(W, /obj/item/firearm/incidenary/flamethrower/backtank))
			if (linkedflamer && (linkedflamer in src.contents))
				boutput(user, SPAN_NOTICE("There already a flamethrower stowed in your [src.name]."))
			else
				var/obj/item/firearm/incidenary/flamethrower/backtank/flamer = W
	...
	proc/insert_flamer(var/obj/item/firearm/incidenary/flamethrower/backtank/flamer, var/mob/user)
...
ABSTRACT_TYPE(/obj/item/firearm/incidenary/flamethrower/backtank)
/obj/item/firearm/incidenary/flamethrower/backtank
...
/obj/item/firearm/incidenary/flamethrower/backtank/napalm
	New()
		..()
		gastank.reagents.add_reagent("syndicate_napalm", 4000)

/obj/item/firearm/incidenary/flamethrower/assembled/New()
	..()
...
/obj/item/firearm/incidenary/flamethrower/assembled/disposing()
...
/obj/item/firearm/incidenary/flamethrower/assembled/loaded
...
/obj/item/firearm/incidenary/flamethrower/assembled/loaded/napalm
	icon_state = "flamethrower_oxy_fuel"
...
/obj/item/flamethrower_construction/proc/completition(var/atom/to_combine_atom, var/mob/user)
	...
	user.u_equip(src)
	var/obj/item/firearm/incidenary/flamethrower/assembled/new_flamethrower = new/obj/item/firearm/incidenary/flamethrower/assembled
...
/obj/item/firearm/incidenary/flamethrower/process()
	if(!lit)
...
/obj/item/firearm/incidenary/flamethrower/proc/swap_any(obj/item/F, mob/user as mob)
	if(!istype(F) || !F)
...
/obj/item/firearm/incidenary/flamethrower/assembled/attackby(obj/item/W, mob/user as mob)
	...
	// PantsNote: Flamethrower disassmbly.
		...
		var/obj/item/firearm/incidenary/flamethrower/assembled/S = src
...
/obj/item/firearm/incidenary/flamethrower/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	var/mob/user
	if(ismob(usr))
		user = usr
...
/obj/item/firearm/incidenary/flamethrower/assembled/attack_self(mob/user as mob)
	if(user.stat || user.restrained() || user.lying)
		return
	src.ui_interact(user)

/obj/item/firearm/incidenary/flamethrower/backtank/attack_self(mob/user as mob)
	if(user.stat || user.restrained() || user.lying)
		return
	src.lit = !src.lit
	if(src.lit)
...
/obj/item/firearm/incidenary/flamethrower/ui_interact(mob/user, datum/tgui/ui)
	if (src.fueltank)
...
/obj/item/firearm/incidenary/flamethrower/ui_data(mob/user)
	. = list(
		"lit" = src.lit,
...
/obj/item/firearm/incidenary/flamethrower/move_trigger(var/mob/M, kindof)
	if (..())
